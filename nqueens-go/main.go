...
func loadConfig() Configuration {
	var (
		population     = flag.Int("population", 1000, "Sets the population")
...
func run(cfg Configuration) error {
	...
	for gen := 0; gen <= cfg.MaxGenerations; gen++ {
		...
		population = selectNextGeneration(population, totalFitness, cfg.MutationChance)
...
func selectNextGeneration(population []DNA, totalFitness float64, mutationChance float64) []DNA {
	n := len(population)
	nextGen := make([]DNA, n)
